...
///
/// <para>
/// Vendored Dock keeps the upstream base type (<see cref="Window"/>) — the
/// SatelliteWindows + Dock unification happens by SatelliteWindow inheriting
/// FROM HostWindow (in SatelliteWindows.Avalonia), so consumers receive a
/// single apex class (<c>SatelliteWindow</c>) that carries both snap
/// behaviour AND Dock host-window machinery. Dock framework code keeps
/// instantiating <c>HostWindow</c> for drag-out, but the
/// <see cref="DockControl.HostWindowFactory"/> hook lets the host return a
/// <c>SatelliteWindow</c> instance instead.
/// </para>
/// </summary>
